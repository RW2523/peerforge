...
def _fetch_material_chunks(debate_id: str, limit: int = 40) -> List[Dict[str, Any]]:
    ...
    with get_db_connection() as conn:
        ...
        cur.execute(
            """
            SELECT mc.chunk_id, mc.chunk_text, mc.chunk_metadata,
    ...
    for r in rows:
        ...
        chunks.append({
            ...
            "_norm": " ".join(_WORD_RE.findall(text.lower())),
            "_tokens": set(t for t in _WORD_RE.findall(text.lower()) if len(t) > 3),
        })
    return chunks


def _containment(candidate: str, chunk_tokens: set) -> float:
    toks = [t for t in _WORD_RE.findall(candidate.lower()) if len(t) > 3]
    if len(toks) < 4:
        return 0.0
    return sum(1 for t in toks if t in chunk_tokens) / len(toks)


def ground_message(debate_id: str, text: str, max_citations: int = 3) -> Dict[str, Any]:
    """Ground a live panel message against the session's material chunks.

    Pure string matching — no LLM call, so it adds no per-turn latency worth
    noting. Candidate claims are the message's substantial sentences plus any
    quoted spans; each is matched by exact normalized substring first, then by
    token containment. Returns the top citations (deduped by chunk) with the
    chunk's sha256 re-verified, ready to embed in the event payload.
    """
    empty = {"citations": [], "summary": {"candidates": 0, "grounded": 0}}
    if not (text or "").strip():
        return empty
    chunks = _fetch_material_chunks(debate_id)
    if not chunks:
        return empty

    sentences = [s.strip() for s in _SENTENCE_SPLIT_RE.split(text) if len(s.strip()) >= 60]
    quotes = [q.strip() for q in _QUOTE_RE.findall(text)]
    candidates = quotes + sentences
    if not candidates:
        return empty

    matches: List[Dict[str, Any]] = []
    for cand in candidates:
        norm = " ".join(_WORD_RE.findall(cand.lower()))
        best, best_score, best_method = None, 0.0, "none"
        for c in chunks:
            if norm and norm in c["_norm"]:
                best, best_score, best_method = c, 1.0, "exact_substring"
                break
            score = _containment(cand, c["_tokens"])
            if score > best_score:
                best, best_score, best_method = c, score, "token_overlap"
        if best is not None and (best_method == "exact_substring" or best_score >= 0.55):
            matches.append({"chunk": best, "claim": cand, "score": best_score, "method": best_method})

    # Dedupe by chunk, keep the strongest claim per chunk, cap the list.
    by_chunk: Dict[str, Dict[str, Any]] = {}
    for m in matches:
        cid = m["chunk"]["chunk_id"]
        if cid not in by_chunk or m["score"] > by_chunk[cid]["score"]:
            by_chunk[cid] = m
    top = sorted(by_chunk.values(), key=lambda m: m["score"], reverse=True)[:max_citations]
    ...
    for m in top:
        ...
        highlight = _locate_excerpt(m["claim"], excerpt)
        citations.append({
            "chunk_id": c["chunk_id"],
            ...
            "page_num": c["page_num"],
            "sha256": c["sha256"],
            "sha256_verified": _verify_sha256(chunk_text, c["sha256"]),
            "method": m["method"],
    ...
    return {
        "citations": citations,
        "summary": {"candidates": len(candidates), "grounded": len(by_chunk)},
    }
