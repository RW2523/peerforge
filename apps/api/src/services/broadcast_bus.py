...
class BroadcastBus:
    ...
    async def start(self, deliver: Callable[[str, Dict[str, Any]], Awaitable[None]]) -> None:
        ...
        try:
            ...
            # Single-instance deployments are the common case and must not be
            # broken by an absent Redis.
            self._degraded = True
            logger.warning(
                "Broadcast bus unavailable, falling back to local-only delivery: %s", exc
            )
            ...
            async for raw in self._pubsub.listen():
                ...
                if self._deliver:
                    try:
                        ...
                        logger.warning("Local delivery of a relayed message failed: %s", exc)
        except asyncio.CancelledError:
            raise
        except Exception as exc:
            logger.warning("Broadcast listener stopped: %s", exc)
